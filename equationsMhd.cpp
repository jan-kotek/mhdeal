...
template <int dim>
void Equations<EquationsTypeMhd, dim>::compute_flux_matrix(const values_vector& W, std::array <std::array <double, dim>, n_components >& flux, const Parameters<dim>& parameters, double resistivity, Tensor<2, dim> Bgrad)
{
    const double total_pressure = compute_total_pressure(W, parameters);
    const double oneOverRho = 1. / W[0];
    const double UB = (W[1] * W[5] + W[2] * W[6] + W[3] * W[7]) * oneOverRho;
    ...
    // 
    //Vector JR=rezistivita * rotB;//indexy 0,1,2 - x,y,z
    //std::cout << "Bgrad" << Bgrad;
    std::array<double, dim> Bcurl = { Bgrad[2][1] - Bgrad[1][2], Bgrad[0][2] - Bgrad[2][0], Bgrad[1][0] - Bgrad[0][1] };
    //Tensor<1,dim> JR = resistivity * Bcurl;
    //std::array<double, dim> JR = { 0.,0.,0.};
    std::array<double, dim> JR = { Bcurl[0]*resistivity, Bcurl[1]*resistivity, Bcurl[2]*resistivity};
    ...
    flux[2][0] = (W[1] * W[2] * oneOverRho) - W[5] * W[6];
    flux[3][0] = (W[1] * W[3] * oneOverRho) - W[5] * W[7];
    flux[4][0] = (W[4] + total_pressure) * (W[1] * oneOverRho) - (W[5] * UB)+JR[1] * W[7] - JR[2] * W[6];//+JR[1]*W[7]-JR[2]*W[6]
    flux[5][0] = 0.0;
    flux[6][0] = ((W[1] * oneOverRho) * W[6]) - ((W[2] * oneOverRho) * W[5]) - JR[2];// -JR[2]
    flux[7][0] = ((W[1] * oneOverRho) * W[7]) - ((W[3] * oneOverRho) * W[5]) + JR[1];// +JR[1]

    flux[0][1] = W[2];
    flux[1][1] = (W[2] * W[1] * oneOverRho) - W[6] * W[5];
    flux[2][1] = (W[2] * W[2] * oneOverRho) - W[6] * W[6] + total_pressure;
    flux[3][1] = (W[2] * W[3] * oneOverRho) - W[6] * W[7];
    flux[4][1] = (W[4] + total_pressure) * (W[2] * oneOverRho) - (W[6] * UB) - JR[0] * W[6] + JR[2] * W[5];//-JR[0]*W[6]+JR[2]*W[5]
    flux[5][1] = ((W[2] * oneOverRho) * W[5]) - ((W[1] * oneOverRho) * W[6]) + JR[2];//+JR[2]
    flux[6][1] = 0.0;
    flux[7][1] = ((W[2] * oneOverRho) * W[7]) - ((W[3] * oneOverRho) * W[6]) - JR[0];//-JR[0]

    flux[0][2] = W[3];
    flux[1][2] = (W[3] * W[1] * oneOverRho) - W[7] * W[5];
    flux[2][2] = (W[3] * W[2] * oneOverRho) - W[7] * W[6];
    flux[3][2] = (W[3] * W[3] * oneOverRho) - W[7] * W[7] + total_pressure;
    flux[4][2] = (W[4] + total_pressure) * (W[3] * oneOverRho) - (W[7] * UB) - JR[1] * W[5] + JR[0] * W[6];//-JR[1]*W[5]+JR[0]*W[6]
    flux[5][2] = ((W[3] * oneOverRho) * W[5]) - ((W[1] * oneOverRho) * W[7]) - JR[1];//-JR[1]
    flux[6][2] = ((W[3] * oneOverRho) * W[6]) - ((W[2] * oneOverRho) * W[7]) + JR[0];//+JR[0]
    flux[7][2] = 0.0;
}
...
template <int dim>
void Equations<EquationsTypeMhd, dim>::compute_flux_vector(const Tensor<1, dim>& normal, const values_vector& W, std::array <double, n_components >& flux, const Parameters<dim>& parameters, double resistivity, Tensor<2, dim> Bgrad)
{
    const double E = W[4];
    const double total_pressure = compute_total_pressure(W, parameters);
    const double oneOverRho = 1. / W[0];
    ...
    case 0:
        ...
        flux[2] = (W[1] * W[2] * oneOverRho) - W[5] * W[6];
        flux[3] = (W[1] * W[3] * oneOverRho) - W[5] * W[7];
        flux[4] = (E + total_pressure) * (W[1] * oneOverRho) - (W[5] * UB) + JR[1] * W[7] - JR[2] * W[6];
        flux[5] = 0.0;
        flux[6] = ((W[1] * oneOverRho) * W[6]) - ((W[2] * oneOverRho) * W[5]) - JR[2];
        flux[7] = ((W[1] * oneOverRho) * W[7]) - ((W[3] * oneOverRho) * W[5]) + JR[1];
        break;
    case 1:
        flux[0] = W[2];
        flux[1] = (W[2] * W[1] * oneOverRho) - W[6] * W[5];
        flux[2] = (W[2] * W[2] * oneOverRho) - W[6] * W[6] + total_pressure;
        flux[3] = (W[2] * W[3] * oneOverRho) - W[6] * W[7];
        flux[4] = (E + total_pressure) * (W[2] * oneOverRho) - (W[6] * UB) - JR[0] * W[6] + JR[2] * W[5];
        flux[5] = ((W[2] * oneOverRho) * W[5]) - ((W[1] * oneOverRho) * W[6]) + JR[2];
        flux[6] = 0.0;
        flux[7] = ((W[2] * oneOverRho) * W[7]) - ((W[3] * oneOverRho) * W[6]) - JR[0];
        break;
    case 2:
        flux[0] = W[3];
        flux[1] = (W[3] * W[1] * oneOverRho) - W[7] * W[5];
        flux[2] = (W[3] * W[2] * oneOverRho) - W[7] * W[6];
        flux[3] = (W[3] * W[3] * oneOverRho) - W[7] * W[7] + total_pressure;
        flux[4] = (E + total_pressure) * (W[3] * oneOverRho) - (W[7] * UB) - JR[1] * W[5] + JR[0] * W[6];
        flux[5] = ((W[3] * oneOverRho) * W[5]) - ((W[1] * oneOverRho) * W[7]) - JR[1];
        flux[6] = ((W[3] * oneOverRho) * W[6]) - ((W[2] * oneOverRho) * W[7]) + JR[0];
        flux[7] = 0.0;
    }
    for (unsigned int d = 0; d < n_components; ++d)
        flux[d] *= sign;
