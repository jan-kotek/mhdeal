  ...
  const n_comp_array &Wminus_, n_comp_array &normal_flux, double& max_speed,double resistivity) const
{
  ...
  Fl[4] = (ul[4] + ptl) * (ul[1] * hl[0]) - (ul[5] * hl[0] * (ul[1] * ul[5] + ul[2] * ul[6] + ul[3] * ul[7]));
  Fl[5] = 0.0;
  Fl[6] = -E3;
  Fl[7] = E2;
  ...
  Fr[4] = (ur[4] + ptr) * (ur[1] * hr[0]) - (ur[5] * hr[0] * (ur[1] * ur[5] + ur[2] * ur[6] + ur[3] * ur[7]));
  Fr[5] = 0.0;
  Fr[6] = -E3;
  Fr[7] = E2;
  ...
  // Sl*, Sr*
  spd[1] = spd[2] - fabs(ul[5]) / sqrtdl;
  spd[3] = spd[2] + fabs(ur[5]) / sqrtdr;
  ...
  if (fabs(cl) < SMALL * ptst) {
    ...
    cm = (ul[0] * sdl * sdl - (ul[5] * ul[5])) * cl;
    Ulst[6] = ul[6] * cm;
    Ulst[7] = ul[7] * cm;
  }
  vbstl = (Ulst[1] * ul[5] + Ulst[2] * Ulst[6] + Ulst[3] * Ulst[7]) / Ulst[0];
  Ulst[4] = (sdl * ul[4] - ptl * ul[1] * hl[0] + ptst * spd[2] + ul[5] *
    ((ul[1] * ul[5] + ul[2] * ul[6] + ul[3] * ul[7]) * hl[0] - vbstl)) / sdml;

  // F*_R
  Urst[1] = Urst[0] * spd[2];
  Urst[5] = ur[5];
  cl = ur[0] * sdr * sdmr - (ur[5] * ur[5]);
  if (fabs(cl) < SMALL * ptst) {
    ...
    cm = (ur[0] * sdr * sdr - (ur[5] * ur[5])) * cl;
    Urst[6] = ur[6] * cm;
    Urst[7] = ur[7] * cm;
  }
  vbstr = (Urst[1] * ur[5] + Urst[2] * Urst[6] + Urst[3] * Urst[7]) / Urst[0];
  Urst[4] = (sdr * ur[4] - ptr * ur[1] * hr[0] + ptst * spd[2] + ur[5] *
    ((ur[1] * ur[5] + ur[2] * ur[6] + ur[3] * ur[7]) * hr[0] - vbstr)) / sdmr;
  ...
  if (0.5 * Bx2 / ptst < SMALL) {
    ...
    Uldst[4] = Ulst[4] - sqrtdl * Bsgn * (vbstl - spd[2] * ul[5] - (Uldst[2] * Uldst[6] + Uldst[3] * Uldst[7]) / Uldst[0]);
    Urdst[4] = Urst[4] + sqrtdr * Bsgn * (vbstr - spd[2] * ur[5] - (Urdst[2] * Urdst[6] + Urdst[3] * Urdst[7]) / Urdst[0]);
  }
