...
{
  double pressure = 5. / (12. * My_PI);
  double B_0 = std::sqrt(1. / (4. * My_PI));
  for (unsigned int i = 0; i < points.size(); ++i)
  {
    result[i][0] = 25. / (36. * My_PI);
    result[i][1] = -result[i][0] * sin(2. * My_PI * points[i][1]);
    result[i][2] = result[i][0] * sin(2. * My_PI * points[i][0]);
    result[i][3] = 0.;
    result[i][5] = -B_0 * sin(2. * My_PI * points[i][1]);
    result[i][6] = B_0 * sin(4. * My_PI * points[i][0]);
    result[i][7] = 0.0;
