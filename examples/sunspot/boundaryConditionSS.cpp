...
template <int dim>
BoundaryConditionSSWithVortices<dim>::BoundaryConditionSSWithVortices(Parameters<dim>& parameters, SSParameters& cs_parameters) :
...
template <int dim>
void BoundaryConditionSSWithVortices<dim>::bc_vector_value(int boundary_no, const Point<dim> &point, const Tensor<1, dim> &normal, 
  values_vector &result, const grad_vector &grads, const values_vector &values, double time, typename DoFHandler<dim>::active_cell_iterator&) const
{
  ...
  result[0] = values[0];//density wave
  result[1] = 0;
  result[2] = 0.;
  result[3] = 0.0;
...
template <int dim>
BoundaryConditionSSFree<dim>::BoundaryConditionSSFree(Parameters<dim>& parameters, SSParameters& cs_parameters) :
  BoundaryCondition<EquationsTypeMhd, dim>(parameters)
{
}

template <int dim>
void BoundaryConditionSSFree<dim>::bc_vector_value(int boundary_no, const Point<dim> &point, const Tensor<1, dim> &normal, 
  values_vector &result, const grad_vector &grads, const values_vector &values, double time, typename DoFHandler<dim>::active_cell_iterator&) const
{
  for (unsigned int di = 0; di < Equations<EquationsTypeMhd, dim>::n_components; ++di)
    result[di] = values[di];
...
template <int dim>
BoundaryConditionSSTest<dim>::BoundaryConditionSSTest(Parameters<dim>& parameters, SSParameters& cs_parameters) :
  BoundaryCondition<EquationsTypeMhd, dim>(parameters), cs_parameters(cs_parameters)
{
}

template <int dim>
void BoundaryConditionSSTest<dim>::bc_vector_value(int boundary_no, const Point<dim> &point, const Tensor<1, dim> &normal, 
...
template <int dim>
BoundaryConditionSSInitialState<dim>::BoundaryConditionSSInitialState(Parameters<dim>& parameters, SSParameters& cs_parameters) :
  BoundaryCondition<EquationsTypeMhd, dim>(parameters), cs_parameters(cs_parameters),
...
template <int dim>
void BoundaryConditionSSInitialState<dim>::bc_vector_value(int boundary_no, const Point<dim> &p, const Tensor<1, dim> &normal, 
  values_vector &result, const grad_vector &grads, const values_vector &values, double time, typename DoFHandler<dim>::active_cell_iterator& cell) const
{
...
template class BoundaryConditionSSWithVortices<3>;
template class BoundaryConditionSSFree<3>;
template class BoundaryConditionSSInitialState<3>;
template class BoundaryConditionSSTest<3>;
