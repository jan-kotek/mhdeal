...
template <int dim>
BoundaryConditionSlab<dim>::BoundaryConditionSlab(Parameters<dim>& parameters, CSParameters& cs_parameters) :
	BoundaryCondition<EquationsTypeMhd, dim>(parameters)
{
}

template <int dim>
void BoundaryConditionSlab<dim>::bc_vector_value(int boundary_no, const Point<dim>& point, const Tensor<1, dim>& normal,
  ...
  values_vector &result, const grad_vector &grads, const values_vector &values, double time, typename DoFHandler<dim>::active_cell_iterator&) const
{
  ...
  result[0] = 0.; //std::sin(60*time)*std::sin(60 * time);//density wave
  result[1] = 0;
  result[2] = 0.;
  result[3] = 0.0;
